...
def lstm_cell_forward(xt, a_prev, c_prev, parameters):
    ...
    it = sigmoid(np.matmul(Wf, concat) + bi)
    cct = np.tanh(np.matmul(Wf, concat) + bc)
    c_next = ft*c_prev + it*cct
    ot = sigmoid(np.matmul(Wf, concat) + bo)
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    dcct = (dc_next*it + tmp*it*da_next) * (1-cct**2)
    dit = (dc_next*cct + tmp*cct*da_next)*it*(1-it)
    dft = (dc_next*c_prev+tmp*c_prev*da_next)*ft*(1-ft)
