...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = np.sqrt(np.abs(1-normalize(mu_orth)**2))*(project(e_w1,bias_axis)-mu_B)/np.linalg.norm(np.abs(project(e_w1, bias_axis)-mu_B))
    corrected_e_w2B = np.sqrt(np.abs(1-normalize(mu_orth)**2))*(project(e_w2,bias_axis)-mu_B)/np.linalg.norm(np.abs(project(e_w2, bias_axis)-mu_B))
